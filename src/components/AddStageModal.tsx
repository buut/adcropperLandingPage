...
interface AddStageModalProps {
    isOpen: boolean;
    onClose: () => void;
    onAdd: (stages: { name: string, width: number, height: number }[]) => void;
}
...
const AddStageModal: React.FC<AddStageModalProps> = ({ isOpen, onClose, onAdd }) => {
    const [namePrefix, setNamePrefix] = useState('New Stage');
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (selectedTargets.length > 0) {
            const stagesToAdd = selectedTargets.map(target => ({
                name: `${target.width}x${target.height}`,
                width: target.width,
            ...
            <div className="relative w-full max-w-4xl bg-white rounded-2xl shadow-2xl overflow-hidden border border-gray-100 flex flex-col animate-in fade-in zoom-in duration-200 h-[85vh]">
                <div className="p-6 border-b border-gray-100 bg-gray-50/50 flex items-center justify-between shrink-0">
                    <div className="flex items-center gap-3">
                        ...
                        <div className="flex flex-col">
                            <h2 className="text-xl font-bold text-gray-800">Add New Stages</h2>
                            <span className="text-[10px] text-gray-400 font-bold uppercase tracking-widest">Select multiple sizes to add new stages</span>
                        </div>
                    </div>
                    <button 
                        onClick={onClose}
                ...
                <div className="p-8 flex-1 overflow-y-auto custom-scrollbar">
                    <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        ...
                        <div className="flex flex-col gap-6 lg:border-r border-gray-100 lg:pr-8">
                            {/* Stage Name Prefix removed as per request */}
                            ...
                            <div className="flex flex-col gap-3 mt-4">
                                ...
                                <div className="p-4 bg-primary/5 rounded-xl border border-primary/10">
                                    <div className="flex items-center justify-between mb-2">
                                        <span className="text-sm font-medium text-gray-600">Total Stages:</span>
                ...
                <div className="p-6 border-t border-gray-100 bg-gray-50/50 flex items-center justify-end gap-4 shrink-0">
                    ...
                    >
                        ...
                        Add Stages ({selectedTargets.length})
                    </button>
                </div>
            </div>
        </div>
